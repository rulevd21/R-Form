...
Защищённый read-only шлюз между `RFORM_MASTER_DATA_v1` и приватным Streamlit Content Control. Не требует Google Cloud Console, service account или OAuth-клиента.
...
- код не содержит операций записи в Google Sheets.
...
После сохранения секретов приложение должно показать источник `APPS SCRIPT / READ ONLY`. Если live-запрос настроен, но отклонён, приложение останавливается и не подменяет ошибку demo-данными.

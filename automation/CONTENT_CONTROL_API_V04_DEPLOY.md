...
- Streamlit v0.4.1 в ветке `agent/content-control-streamlit-readonly`;
...
- шапка показывает `ЕЖЕДНЕВНЫЙ РЕЖИМ · v0.4.1`;
- диагностика показывает `event.review`, `event.decision`, `event.media`;
- раздел `Сегодня` показывает одно предложение без ошибки;
- кнопки `Добавить в публикации`, `Сохранить для Weekly` и `Не использовать` активны.
...
3. выберите `Сохранить для Weekly`;
4. обновите данные;

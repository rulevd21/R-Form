// R/Form Telegram Autopost v0.3.1
// Backward-compatible with v0.3. Adds Preview_Review_Hash guard for Channel Control v0.3.
...
function rformTgPreflight() {
  ...
  return {
    ok: true, mode: 'READ_ONLY_PREFLIGHT', botUsername: '@' + me.username,
...
function rformTgProcessQueue() {
  ...
  for (let r = 1; r < data.length; r++) {
    ...
    const rowNumber = r + 1;
    // v0.3.1 safety gate: if Channel Control has stored a verified hash, it MUST still match immediately before posting.
...
function rformTgReviewHashPasses_(sheet,rowNumber,row,col,mode,text) {
  if (col['Preview_Review_Hash'] === undefined) return true; // backward compatible
  const stored = String(rformTgValue_(row,col,'Preview_Review_Hash') || '').trim();
  if (!stored) return true; // legacy rows are not blocked
...
function rformTgBuildReviewHash_(mode,text,visualUrl) {
  const visuals = mode === 'TEXT_ONLY' ? [] : rformTgVisualSignatures_(visualUrl).slice(0,10);
  const canonical = JSON.stringify({mode:String(mode||'TEXT_ONLY').toUpperCase(),text:String(text||''),visualUrl:String(visualUrl||'').trim(),visuals:visuals});
...
function rformTgVisualSignatures_(url) {
  if (!url) return [];
  const id = rformTgExtractDriveId_(url);
  if (!id) return [];
  const folder = DriveApp.getFolderById(id), out=[], files=folder.getFiles();
  while(files.hasNext()){
    const f=files.next(), mime=f.getMimeType();
    if(mime===MimeType.PNG || mime==='image/jpeg') out.push({id:f.getId(),name:f.getName(),size:f.getSize(),updated:f.getLastUpdated().toISOString()});
  }

...
const RFORM_PROD_PREFLIGHT_VERSION = '0.4.0-rc1';
...
function getProductionPreflightState() {
  ...
  const featureSchema = RFORM_PROD_FEATURE_SHEETS.reduce((out, name) => {
    out[name] = Boolean(sheets[name]);
    return out;
  }, {});
  ...
  return {
    app: {
      ...
      environment: 'PRODUCTION_RC_READ_ONLY',
      appVersion: RFORM_PROD_PREFLIGHT_VERSION,
      timezone,
    ...
    datastore: {
      title,
      exactTitleMatch: title === RFORM_PROD_EXACT_MASTER_TITLE,
      missingRequiredSheets: missingRequired,
      featureSchema,
      schemaReadyForFastPath: RFORM_PROD_FEATURE_SHEETS.every(name => featureSchema[name])
    },
    today,
    nutrition,
    training,
    gate: {
      status: missingRequired.length ? 'BLOCKED_REQUIRED_SCHEMA' : 'READ_ONLY_PREFLIGHT_READY',
...
function readProductionToday_(sheet, dateKey, timezone) {
  ...
  const headers = prodHeaderMap_(sheet);
  const required = ['Day_ID','Date','Day_Type','Morning_Weight','Weight_7D_Average','Sleep_Hours','Sleep_Quality','Readiness','Day_Status'];
  ...
  return {
    state: String(prodValue_(values, headers, 'Day_Status') || 'UNKNOWN').trim().toUpperCase(),
    date: dateKey,
    dayId: String(prodValue_(values, headers, 'Day_ID') || '').trim(),
    dayType: String(prodValue_(values, headers, 'Day_Type') || '').trim(),
    morningWeight: prodNumberOrBlank_(prodValue_(values, headers, 'Morning_Weight')),
    weight7dAverage: prodNumberOrBlank_(prodValue_(values, headers, 'Weight_7D_Average')),
    sleepHours: prodNumberOrBlank_(prodValue_(values, headers, 'Sleep_Hours')),
    sleepQuality: prodNumberOrBlank_(prodValue_(values, headers, 'Sleep_Quality')),
    readiness: prodNumberOrBlank_(prodValue_(values, headers, 'Readiness'))
  };
}
...
function readProductionNutrition_(sheet, today, dateKey, timezone) {
  ...
  const headers = prodHeaderMap_(sheet);
  const required = ['Day_ID','Date','Meal_Count','Calories_Min','Calories_Max','Protein_Min','Protein_Max','Fat_Min','Fat_Max','Carbs_Min','Carbs_Max','Status'];
  const missing = required.filter(name => !headers[name]);
  ...
  const values = sheet.getRange(row, 1, 1, sheet.getLastColumn()).getValues()[0];
  return {
    status: String(prodValue_(values, headers, 'Status') || 'UNKNOWN').trim().toUpperCase(),
    date: dateKey,
    mealCount: Number(prodValue_(values, headers, 'Meal_Count')) || 0,
    caloriesMin: prodNumberOrBlank_(prodValue_(values, headers, 'Calories_Min')),
    caloriesMax: prodNumberOrBlank_(prodValue_(values, headers, 'Calories_Max')),
    proteinMin: prodNumberOrBlank_(prodValue_(values, headers, 'Protein_Min')),
    proteinMax: prodNumberOrBlank_(prodValue_(values, headers, 'Protein_Max')),
    fatMin: prodNumberOrBlank_(prodValue_(values, headers, 'Fat_Min')),
    fatMax: prodNumberOrBlank_(prodValue_(values, headers, 'Fat_Max')),
    carbsMin: prodNumberOrBlank_(prodValue_(values, headers, 'Carbs_Min')),
    carbsMax: prodNumberOrBlank_(prodValue_(values, headers, 'Carbs_Max'))
  };
}
...
function readProductionTraining_(sheet, dateKey, timezone, configuredUrl) {
  ...
  const headers = prodHeaderMap_(sheet);
  const dateColumn = headers.Date || headers.Session_Date;
  ...
  if (lastRow >= 2) {
    ...
    values.forEach((row, index) => {
      if (prodNormalizeDate_(row[dateColumn - 1], timezone) !== dateKey) return;
      rows.push({
        row: index + 2,
        sessionId: headers.Session_ID ? String(row[headers.Session_ID - 1] || '').trim() : '',
        sessionType: headers.Session_Type ? String(row[headers.Session_Type - 1] || '').trim() : '',
        sessionStatus: headers.Session_Status ? String(row[headers.Session_Status - 1] || '').trim() : ''
      });
    });
  }
  ...
  return {
    ...
    note: 'Training launch remains disabled in production RC until the current Training Mobile URL is independently verified.'
  };
}

function prodHeaderMap_(sheet) {
...
function prodFindDateRow_(sheet, column, dateKey, timezone) {
  const lastRow = sheet.getLastRow();
  if (!column || lastRow < 2) return 0;
  const values = sheet.getRange(2, column, lastRow - 1, 1).getValues();
  for (let i = values.length - 1; i >= 0; i -= 1) {
    if (prodNormalizeDate_(values[i][0], timezone) === dateKey) return i + 2;
  }
  return 0;
}

function prodFindExactRow_(sheet, column, expected) {
  const lastRow = sheet.getLastRow();
  if (!column || lastRow < 2) return 0;
  const values = sheet.getRange(2, column, lastRow - 1, 1).getDisplayValues();
  for (let i = values.length - 1; i >= 0; i -= 1) {
    if (String(values[i][0] || '').trim() === String(expected || '').trim()) return i + 2;
  }
  return 0;
}

function prodNormalizeDate_(value, timezone) {
...
function prodValue_(row, headers, name) {
  const column = headers[name];
  return column ? row[column - 1] : '';
}

function prodNumberOrBlank_(value) {

...
function saveMealTemplate(payload) {
  ...
  requireHeaders_(rawHeaders, [
    'Meal_ID','Meal_Type','Food_Name_Original','Amount','Unit','Source','Status','Duplicate_Flag'
  ], 'NUTRITION_RAW');
  requireHeaders_(catalogHeaders, [
    'Food_ID','Display_Name','Brand','Basis','Verified_By_User','Status','Duplicate_Flag'
  ], 'FOOD_CATALOG');
  requireHeaders_(templateHeaders, [
    'Meal_Template_ID','Template_Name','Meal_Type','Component_Order','Food_ID','Default_Amount','Unit','Status','Last_Used_At'
  ], 'MEAL_TEMPLATES');
  ...
  try {
    ...
    if (existingTemplateId) {
      inboxRow = writeTemplateInbox_(
        inbox, inboxHeaders, input, inboxId, existingTemplateId, sourceMeal, now, sheetDateSerial, 'SKIPPED',
        `Existing ACTIVE template has the same Meal_Type and ordered Food_ID/Amount/Unit signature.`
      );
    ...
    const rows = sourceMeal.components.map((component, index) => [
      templateId,
      templateName,
      sourceMeal.mealType,
      index + 1,
    ...
    inboxRow = writeTemplateInbox_(
      inbox, inboxHeaders, input, inboxId, templateId, sourceMeal, now, sheetDateSerial, 'APPLIED',
      `Created ${templateRowCount} component row(s); nutrition values are not stored in MEAL_TEMPLATES.`
    );
...
function resolveTemplateSourceMeal_(raw, rawHeaders, catalog, catalogHeaders, sourceMealId) {
  ...
  if (catalogLastRow >= 2) {
    ...
    rows.forEach(row => {
      ...
      catalogMap[foodId] = {
        foodId,
        displayName: String(valueByHeader_(row, catalogHeaders, 'Display_Name') || '').trim() || foodId,
        brand: String(valueByHeader_(row, catalogHeaders, 'Brand') || '').trim(),
        basis: String(valueByHeader_(row, catalogHeaders, 'Basis') || '').trim()
      };
    });
  }
  ...
  if (lastRow >= 2) {
    ...
    rows.forEach((row, index) => {
      ...
      matches.push({
        row: index + 2,
        foodId,
        displayName: catalogMap[foodId].displayName,
        brand: catalogMap[foodId].brand,
        amount,
        unit,
        mealType: String(valueByHeader_(row, rawHeaders, 'Meal_Type') || '').trim()
      });
    });
  }
...
function buildTemplateSignature_(mealType, components) {
  return [String(mealType || '').trim().toUpperCase()]
    .concat(components.map(component => `${component.foodId}:${normalizeTemplateNumber_(component.amount)}:${component.unit}`))
    .join('|');
}
...
function findActiveTemplateBySignature_(sheet, headers, targetSignature) {
  ...
  rows.forEach(row => {
    ...
    if (!groups[templateId]) groups[templateId] = { mealType: '', components: [] };
    groups[templateId].mealType = String(valueByHeader_(row, headers, 'Meal_Type') || '').trim();
    groups[templateId].components.push({
      order: Number(valueByHeader_(row, headers, 'Component_Order')) || 999,
      foodId: String(valueByHeader_(row, headers, 'Food_ID') || '').trim(),
      amount: Number(valueByHeader_(row, headers, 'Default_Amount')),
      unit: String(valueByHeader_(row, headers, 'Unit') || '').trim()
    });
  });
  ...
  for (const templateId in groups) {
    ...
    if (group.components.some(c => !c.foodId || !Number.isFinite(c.amount) || c.amount <= 0 || !c.unit)) continue;
    const signature = buildTemplateSignature_(group.mealType, group.components);
    if (signature === targetSignature) return templateId;
  }
  return '';
}

function buildMealTemplateId_(dateKey, eventId) {
  return `MT-${String(dateKey).replace(/-/g, '')}-${String(eventId).replace(/-/g, '').slice(0, 8).toUpperCase()}`;
}

function buildDefaultTemplateName_(sourceMeal) {
  const names = sourceMeal.components.slice(0, 2).map(component => component.displayName || component.foodId);
  const suffix = sourceMeal.components.length > 2 ? ' +' : '';
  return `${sourceMeal.mealType} · ${names.join(' + ')}${suffix}`.slice(0, 80);
}

function normalizeTemplateNumber_(value) {
...
function prepareTemplateRows_(sheet, dictionaries, startRow, rowCount, headers) {
  if (startRow > 2) {
    ...
    sheet.getRange(sourceRow, 1, 1, sheet.getLastColumn()).copyTo(
      sheet.getRange(startRow, 1, rowCount, sheet.getLastColumn()),
  ...
  const mealTypeRule = SpreadsheetApp.newDataValidation()
    .requireValueInRange(dictionaries.getRange('H2:H8'), true).setAllowInvalid(false).build();
  const unitRule = SpreadsheetApp.newDataValidation()
    .requireValueInRange(dictionaries.getRange('P2:P10'), true).setAllowInvalid(false).build();
  const statusRule = SpreadsheetApp.newDataValidation()
    .requireValueInRange(dictionaries.getRange('N2:N7'), true).setAllowInvalid(false).build();

  sheet.getRange(startRow, headers.Meal_Type, rowCount, 1).setDataValidation(mealTypeRule);
  sheet.getRange(startRow, headers.Unit, rowCount, 1).setDataValidation(unitRule);
  sheet.getRange(startRow, headers.Status, rowCount, 1).setDataValidation(statusRule);
}
...
function verifyTemplateRows_(sheet, headers, startRow, rowCount, templateId, expectedSignature) {
  ...
  let mealType = '';
  rows.forEach((row, index) => {
    if (String(valueByHeader_(row, headers, 'Meal_Template_ID') || '').trim() !== templateId) throw new Error('VERIFY_FAILED:MEAL_TEMPLATES:Template_ID');
    if (String(valueByHeader_(row, headers, 'Status') || '').trim().toUpperCase() !== 'ACTIVE') throw new Error('VERIFY_FAILED:MEAL_TEMPLATES:Status');
    const order = Number(valueByHeader_(row, headers, 'Component_Order'));
    if (order !== index + 1) throw new Error('VERIFY_FAILED:MEAL_TEMPLATES:Component_Order');
  ...
  const storedSignature = buildTemplateSignature_(mealType, components);
  if (storedSignature !== expectedSignature) throw new Error('VERIFY_FAILED:MEAL_TEMPLATES:SIGNATURE');
}
...
function writeTemplateInbox_(inbox, headers, input, inboxId, templateId, sourceMeal, now, sheetDateSerial, processingStatus, detail) {
  const row = inbox.getLastRow() + 1;
  ensureRows_(inbox, row);
  prepareNewRowsLikePrevious_(inbox, row, 1);
  const values = new Array(18).fill('');
  values[0] = inboxId;
  values[1] = now;
  values[2] = sheetDateSerial;
  values[3] = 'CORRECTION';
  values[4] = `MEAL_TEMPLATE ${sourceMeal.sourceMealId} -> ${templateId}`;
  values[5] = 'MEAL_TEMPLATE';
  values[6] = 'MEAL_TEMPLATES';
  values[7] = templateId;
  values[8] = 'VALID';
  values[9] = '';
  ...
  values[11] = now;
  values[12] = 'OWNER';
  values[13] = RFORM_TEMPLATE_SOURCE;
  values[14] = RFORM_PHASE3C4_VERSION;
  values[15] = '';
  values[16] = `=IF(A${row}="";"";IF(COUNTIF($A$2:$A$5000;A${row})>1;"DUPLICATE";""))`;
  values[17] = `Phase 3C.4 MEAL_TEMPLATE; sourceMeal=${sourceMeal.sourceMealId}; components=${sourceMeal.components.length}; client=${input.clientVersion || 'unknown'}; ${detail}`;
  inbox.getRange(row, 1, 1, 18).setValues([values]);
  inbox.getRange(row, headers.Received_At).setNumberFormat('dd.mm.yyyy hh:mm');
  inbox.getRange(row, headers.Event_Date).setNumberFormat('dd.mm.yyyy');
  inbox.getRange(row, headers.Applied_At).setNumberFormat('dd.mm.yyyy hh:mm');
  SpreadsheetApp.flush();
  return row;
}

function verifyTemplateInbox_(inbox, headers, row, inboxId, templateId, processingStatus, sheetDateSerial, timezone) {
  if (inbox.getRange(row, headers.Inbox_Event_ID).getDisplayValue() !== inboxId) throw new Error('VERIFY_FAILED:INBOX_LOG:Inbox_Event_ID');
  if (String(inbox.getRange(row, headers.Event_Type).getDisplayValue()).trim() !== 'CORRECTION') throw new Error('VERIFY_FAILED:INBOX_LOG:Event_Type');
  if (String(inbox.getRange(row, headers.Parsed_Entity).getDisplayValue()).trim() !== 'MEAL_TEMPLATE') throw new Error('VERIFY_FAILED:INBOX_LOG:Parsed_Entity');
  if (String(inbox.getRange(row, headers.Target_Record_ID).getDisplayValue()).trim() !== templateId) throw new Error('VERIFY_FAILED:INBOX_LOG:Target_Record_ID');
  if (String(inbox.getRange(row, headers.Processing_Status).getDisplayValue()).trim() !== processingStatus) throw new Error('VERIFY_FAILED:INBOX_LOG:Processing_Status');
  verifyCalendarDateCellPhase3_(inbox.getRange(row, headers.Event_Date), sheetDateSerial, timezone, 'INBOX_LOG:Event_Date');
  if (inbox.getRange(row, headers.Duplicate_Flag).getDisplayValue()) throw new Error('VERIFY_FAILED:INBOX_LOG:DUPLICATE');
}

function buildTemplateResult_(status, input, templateId, inboxId) {
  const fresh = getPhase3C4BootstrapState();
  const template = (fresh.fastPaths && fresh.fastPaths.templates || []).find(item => item.templateId === templateId) || null;
  return {
    status,
    ...
    appVersion: RFORM_PHASE3C4_VERSION
  };
}

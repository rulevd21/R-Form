...
function getTrainingStateByDate_(ss, dateKey, dayId, config) {
  ...
  requireHeaders_(
    headers,
    ['Session_ID','Day_ID','Date','Session_Type','Plan_Status','Session_Status'],
    'TRAINING_SESSIONS'
  );

  const row = findLegacyPlannedSessionRow_(
    sessions,
    headers,
    dateKey,
    dayId,
...
function findLegacyPlannedSessionRow_(sessions, headers, dateKey, dayId, timezone) {
  ...
  const values = sessions
    .getRange(2, 1, lastRow - 1, sessions.getLastColumn())
    .getValues();

  for (let i = values.length - 1; i >= 0; i--) {
    const row = values[i];
    const mode = headers.Session_Mode
      ? String(row[headers.Session_Mode - 1] || '').trim().toUpperCase()
      : '';
    ...
    const sameDay =
      dayId &&
      String(row[headers.Day_ID - 1] || '') === String(dayId);

    const sameDate =
      normalizeDateKey_(row[headers.Date - 1], timezone) === dateKey;
...
function getFreeTrainingStateByDate_(sessions, dateKey, dayId, config) {
  ...
  requireHeaders_(
    headers,
    ['Session_ID','Day_ID','Date','Session_Status','Session_Mode'],
    'TRAINING_SESSIONS'
  );
  ...
  for (let i = values.length - 1; i >= 0; i--) {
    ...
    if (
      String(row[headers.Session_Mode - 1] || '')
        .trim()
        .toUpperCase() !== 'FREE'
    ) {
      continue;
    ...
    const sameDate =
      normalizeDateKey_(row[headers.Date - 1], config.timezone) === dateKey;

...
const RFORM_CONFIG_KEYS = Object.freeze({
  ...
  PRODUCTION_WRITE_ACK: 'PRODUCTION_WRITE_ACK'
});

const RFORM_PRODUCTION_DATASTORE_TITLE = 'RFORM_MASTER_DATA_v1';
const RFORM_PRODUCTION_WRITE_ACK = 'RFORM_TRAINING_CHANGES_V0_1';
...
function doGet() {
  ...
  return HtmlService.createHtmlOutput(html)
    .setTitle('R/Form Mobile — Production RC1');
}
...
function getConfig_() {
  ...
  const config = {
    ...
    appVersion: props.getProperty(RFORM_CONFIG_KEYS.APP_VERSION) || '0.1.0-rc1',
    ...
    environment: String(props.getProperty(RFORM_CONFIG_KEYS.APP_ENVIRONMENT) || '').trim().toUpperCase(),
    productionWriteAck: String(props.getProperty(RFORM_CONFIG_KEYS.PRODUCTION_WRITE_ACK) || '').trim()
  };

  if (!config.masterSpreadsheetId) throw new Error('CONFIG_MISSING: MASTER_SPREADSHEET_ID');
  if (config.environment !== 'PRODUCTION') throw new Error('SAFETY_GUARD:EXPECTED_PRODUCTION_ENVIRONMENT');
  if (config.productionWriteAck !== RFORM_PRODUCTION_WRITE_ACK) {
    throw new Error('SAFETY_GUARD:PRODUCTION_WRITE_NOT_ACKNOWLEDGED');
  }
  return config;
}
...
function getMasterSpreadsheet_() {
  ...
  if (title !== RFORM_PRODUCTION_DATASTORE_TITLE) {
...
function buildTrainingLaunchState_(today, config) {
  ...
  return {
    ...
    structuredChangesMode: hasSession ? 'PRODUCTION_FACT_ONLY_RC1' : 'UNAVAILABLE',
    planImmutable: true,

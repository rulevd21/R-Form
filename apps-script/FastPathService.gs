...
/**
 * Phase 3C.1 bootstrap. Read-only addition over the accepted Phase 3A writer.
 * No Favorite/Template writer is exposed at this stage.
 */
...
function getNutritionFastPathState_(config) {
  ...
  requireHeaders_(rawHeaders, [
    'Day_ID','Date','Meal_ID','Meal_Time','Meal_Type','Food_Name_Original','Amount','Unit',
    'Calories_Min','Protein_Min','Fat_Min','Carbs_Min','Source','Created_At','Status','Record_Key','Duplicate_Flag'
  ], 'NUTRITION_RAW');
  requireHeaders_(catalogHeaders, [
    'Food_ID','Display_Name','Brand','Basis','Basis_Amount','Calories','Protein','Fat','Carbs',
    'Verified_By_User','Favorite','Status','Duplicate_Flag'
  ], 'FOOD_CATALOG');
  ...
  const favoriteFoods = Object.keys(catalogMap)
    .map(id => catalogMap[id])
    .filter(food => food.favorite)
    .sort((a, b) => (b.lastUsedSort || 0) - (a.lastUsedSort || 0) || a.displayName.localeCompare(b.displayName, 'ru'))
    .map(publicFastPathFood_);
  ...
  return {
    ...
    limits: {
      recentMeals: RFORM_FASTPATH_RECENT_MEALS,
      recentFoods: RFORM_FASTPATH_RECENT_FOODS
    },
    readOnly: true
  };
}
  ...
  rows.forEach((row, index) => {
    ...
    if (!foodUses[foodId] || sortValue > foodUses[foodId].sortValue) {
      foodUses[foodId] = {
        ...
        unit,
        sortValue,
        usedAt: serializeDateTime_(createdAt, config.timezone)
      };
    }
  });

  const recentMeals = Object.keys(meals)
    .map(id => meals[id])
    ...
    .map(meal => ({
      ...
      nutrition: {
        calories: roundFastPath_(meal.calories),
        protein: roundFastPath_(meal.protein),
        fat: roundFastPath_(meal.fat),
        carbs: roundFastPath_(meal.carbs)
      },
      createdAt: meal.createdAt
    }));

  const recentFoods = Object.keys(foodUses)
    .map(foodId => {
      const use = foodUses[foodId];
      const food = catalogMap[foodId];
      return Object.assign(publicFastPathFood_(food), {
        lastAmount: use.amount,
        lastUnit: use.unit,
        usedAt: use.usedAt,
        sortValue: use.sortValue
      });
    })
    .sort((a, b) => b.sortValue - a.sortValue)
    .map(item => {
...
function buildFastPathTemplates_(sheet, headers, catalogMap, config) {
  ...
  rows.forEach(row => {
    ...
    if (!groups[templateId]) {
      groups[templateId] = {
        templateId,
        templateName: String(valueByHeader_(row, headers, 'Template_Name') || '').trim() || templateId,
        mealType: String(valueByHeader_(row, headers, 'Meal_Type') || '').trim(),
        components: [],
        lastUsedAt: serializeDateTime_(lastUsed, config.timezone),
        sortValue: dateSortValue_(lastUsed)
      };
    }
...
function roundFastPath_(value) {
  return Math.round((Number(value) || 0) * 100) / 100;
}

function formatFastPathNumber_(value) {
  const number = Number(value);

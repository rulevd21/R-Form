...
function startTrainingFreeSession(payload) {
  ...
  try {
    ...
    inboxRow = trainingFreeWriteAudit_(inbox, ih, {
      inboxId, eventDate: trainingFreeDateObject_(input.dateKey), eventType: 'TRAINING_FREE_SESSION_START',
      rawMessage: JSON.stringify(payload), parsedEntity: JSON.stringify({ sessionId, sessionMode: RFORM_TRAINING_FREE_MODE }),
      targetSheet: 'TRAINING_SESSIONS', targetRecordId: sessionId, note: 'FREE training session started.'
    });
    SpreadsheetApp.flush();
...
function completeTrainingFreeSession(payload) {
  ...
  const sh = getHeaderMap_(sessions); const th = getHeaderMap_(sets); const ih = getHeaderMap_(inbox);
  trainingFreeRequireSessionSchema_(sh); trainingFreeRequireSetSchema_(th); trainingFreeRequireInboxSchema_(ih);
  ...
  try {
    ...
    inboxRow = trainingFreeWriteAudit_(inbox, ih, {
      inboxId, eventDate: sessions.getRange(row, sh.Date).getValue(), eventType: 'TRAINING_FREE_SESSION_COMPLETE',
      rawMessage: JSON.stringify(payload), parsedEntity: JSON.stringify({ sessionId, setCount: factRows.length }),
      targetSheet: 'TRAINING_SESSIONS', targetRecordId: sessionId, note: 'FREE training session completed.'
    });
...
function getTrainingFreeSessionState(sessionId) {
  ...
  const sh = getHeaderMap_(sessions); const th = getHeaderMap_(sets);
  trainingFreeRequireSessionSchema_(sh); trainingFreeRequireSetSchema_(th);
  ...
  setRows.forEach(set => {
    ...
    if (!groups[key]) groups[key] = { exerciseInstanceId: key, exerciseCatalogId: set.exerciseCatalogId, exerciseOrder: set.exerciseOrder, exerciseName: set.exerciseName, exerciseNormalized: set.exerciseNormalized, exerciseCategory: set.exerciseCategory, exerciseComment: set.exerciseComment, sets: [] };
  ...
  return {
    ...
    status: String(sv[sh.Session_Status - 1] || ''),
    ...
    canEdit: String(sv[sh.Session_Status - 1] || '') !== RFORM_TRAINING_FREE_CLOSED_STATUS,
...
function trainingFreeRequireSessionSchema_(h) { requireHeaders_(h, ['Session_ID','Day_ID','Date','Session_Type','Actual_Duration','Plan_Status','Session_Status','Duplicate_Flag','Session_Mode','Started_At','Completed_At','Session_Comment'], 'TRAINING_SESSIONS'); }
function trainingFreeRequireSetSchema_(h) { requireHeaders_(h, ['Set_ID','Session_ID','Exercise_Order','Exercise_Name_Original','Exercise_Name_Normalized','Exercise_Category','Set_Type','Set_Number','Weight_Kg','Reps','RIR','Plan_Weight','Plan_Reps','Plan_RIR','Deviation','Comment','Record_Key','Duplicate_Flag','Exercise_Instance_ID','Exercise_Catalog_ID','Load_Value','Load_Unit','Exercise_Comment'], 'TRAINING_SETS'); }
function trainingFreeRequireInboxSchema_(h) { requireHeaders_(h, ['Inbox_Event_ID','Received_At','Event_Date','Event_Type','Raw_Message','Parsed_Entity','Target_Sheet','Target_Record_ID','Validation_Status','Missing_Fields','Processing_Status','Applied_At','Applied_By','Source_Chat','Version','Correction_Of','Duplicate_Flag','Note'], 'INBOX_LOG'); }
...
function trainingFreeValidateEventId_(value) { const id = String(value || '').trim(); if (!/^[0-9a-fA-F-]{32,36}$/.test(id)) throw new Error('VALIDATION:EVENT_ID'); return id; }
function trainingFreeInboxId_(eventId) { return `APP-TRAINING-FREE-${String(eventId).replace(/-/g,'').toUpperCase()}`; }
function trainingFreeBuildSessionId_(dateKey, eventId, sheet, h) { const base = `S-${dateKey.replace(/-/g,'')}-FREE-${String(eventId).replace(/-/g,'').toUpperCase().slice(0,8)}`; if (!trainingFreeFindRowByExact_(sheet,h.Session_ID,base)) return base; return `${base}-${Utilities.getUuid().replace(/-/g,'').toUpperCase().slice(0,4)}`; }
function trainingFreeDateObject_(dateKey) { const m = String(dateKey).match(/^(\d{4})-(\d{2})-(\d{2})$/); if (!m) throw new Error('VALIDATION:DATE'); return new Date(Number(m[1]), Number(m[2])-1, Number(m[3]), 12, 0, 0); }
function trainingFreeResolveDayId_(ss, dateKey) { const daily = ss.getSheetByName('DAILY'); if (!daily) return ''; const h = getHeaderMap_(daily); if (!h.Day_ID || !h.Date) return ''; const row = findRowByDate_(daily, h.Date, dateKey, getConfig_().timezone); return row ? String(daily.getRange(row,h.Day_ID).getDisplayValue() || '') : ''; }
function trainingFreeFindActiveSession_(sheet,h) { const last=sheet.getLastRow(); if(last<2)return null; const values=sheet.getRange(2,1,last-1,sheet.getLastColumn()).getValues(); for(let i=values.length-1;i>=0;i--){ if(String(values[i][h.Session_Mode-1]||'')===RFORM_TRAINING_FREE_MODE && String(values[i][h.Session_Status-1]||'')!==RFORM_TRAINING_FREE_CLOSED_STATUS){ return {row:i+2,sessionId:String(values[i][h.Session_ID-1]||'')}; } } return null; }
function trainingFreeRequireFreeSessionRow_(sheet,h,sessionId,mustBeOpen){ const row=trainingFreeFindRowByExact_(sheet,h.Session_ID,sessionId); if(!row)throw new Error(`TRAINING_SESSION_NOT_FOUND:${sessionId}`); if(String(sheet.getRange(row,h.Session_Mode).getDisplayValue()||'')!==RFORM_TRAINING_FREE_MODE)throw new Error('VALIDATION:SESSION_NOT_FREE'); if(mustBeOpen&&String(sheet.getRange(row,h.Session_Status).getDisplayValue()||'')===RFORM_TRAINING_FREE_CLOSED_STATUS)throw new Error('VALIDATION:SESSION_CLOSED'); return row; }
function trainingFreeRowsByValue_(sheet,column,expected){ const last=sheet.getLastRow(); if(last<2)return[]; const values=sheet.getRange(2,1,last-1,sheet.getLastColumn()).getValues(); const out=[]; values.forEach((v,i)=>{if(String(v[column-1]||'').trim()===String(expected).trim())out.push({row:i+2,values:v});}); return out; }
function trainingFreeFindRowByExact_(sheet,column,expected){ const last=sheet.getLastRow(); if(last<2)return 0; const values=sheet.getRange(2,column,last-1,1).getDisplayValues(); const target=String(expected).trim(); for(let i=0;i<values.length;i++){if(String(values[i][0]||'').trim()===target)return i+2;} return 0; }
function trainingFreeNextRow_(sheet){ const row=sheet.getLastRow()+1; if(row>sheet.getMaxRows())sheet.insertRowsAfter(sheet.getMaxRows(),Math.max(10,row-sheet.getMaxRows())); return row; }
function trainingFreeSetCell_(sheet,row,column,value,format){ if(!column)return; const cell=sheet.getRange(row,column); cell.setValue(value); if(format)cell.setNumberFormat(format); }
function trainingFreeCopyRowScaffold_(sheet,row){ if(row<=2)return; const source=sheet.getRange(row-1,1,1,sheet.getLastColumn()); const target=sheet.getRange(row,1,1,sheet.getLastColumn()); source.copyTo(target,SpreadsheetApp.CopyPasteType.PASTE_FORMAT,false); source.copyTo(target,SpreadsheetApp.CopyPasteType.PASTE_DATA_VALIDATION,false); }
function trainingFreeWriteAudit_(sheet,h,spec){ const row=trainingFreeNextRow_(sheet); trainingFreeCopyRowScaffold_(sheet,row); const now=new Date(); const values={Inbox_Event_ID:spec.inboxId,Received_At:now,Event_Date:spec.eventDate,Event_Type:spec.eventType,Raw_Message:spec.rawMessage||'',Parsed_Entity:spec.parsedEntity||'',Target_Sheet:spec.targetSheet,Target_Record_ID:spec.targetRecordId,Validation_Status:'VALID',Missing_Fields:'',Processing_Status:'APPLIED',Applied_At:now,Applied_By:'RFORM_MOBILE',Source_Chat:'RFORM_MOBILE',Version:RFORM_TRAINING_FREE_VERSION,Correction_Of:'',Duplicate_Flag:'',Note:spec.note||''}; Object.keys(values).forEach(k=>{if(h[k])sheet.getRange(row,h[k]).setValue(values[k]);}); if(h.Received_At)sheet.getRange(row,h.Received_At).setNumberFormat('dd.mm.yyyy hh:mm'); if(h.Event_Date)sheet.getRange(row,h.Event_Date).setNumberFormat('dd.mm.yyyy'); if(h.Applied_At)sheet.getRange(row,h.Applied_At).setNumberFormat('dd.mm.yyyy hh:mm'); return row; }
function trainingFreeSetProjection_(v,h){ return {setId:String(v[h.Set_ID-1]||''),exerciseInstanceId:String(v[h.Exercise_Instance_ID-1]||''),exerciseCatalogId:String(v[h.Exercise_Catalog_ID-1]||''),exerciseOrder:Number(v[h.Exercise_Order-1]||0),exerciseName:String(v[h.Exercise_Name_Original-1]||''),exerciseNormalized:String(v[h.Exercise_Name_Normalized-1]||''),exerciseCategory:String(v[h.Exercise_Category-1]||''),setType:String(v[h.Set_Type-1]||''),setNumber:Number(v[h.Set_Number-1]||0),loadValue:v[h.Load_Value-1]===''?null:Number(v[h.Load_Value-1]),loadUnit:String(v[h.Load_Unit-1]||''),weightKg:v[h.Weight_Kg-1]===''?null:Number(v[h.Weight_Kg-1]),reps:Number(v[h.Reps-1]||0),rir:v[h.RIR-1]===''?null:Number(v[h.RIR-1]),exerciseComment:String(v[h.Exercise_Comment-1]||''),comment:String(v[h.Comment-1]||'')}; }
